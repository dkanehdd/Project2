...
public class BoardDAO {
	...
	public List<BoardDTO> selectListPageSearch(Map<String, Object> map) {
		...
		String query = "" + " SELECT IF(char_LENGTH(title)>20, concat(LEFT(title,20),'...'), title) AS title3 ,"
				...
				+ " INNER JOIN membership m " + " ON b.id=m.id  WHERE attachedfile is null and notice=?";
	...
	public int getTotalRecordCountSearch(Map<String, Object> map) {
		...
		String query = "SELECT COUNT(*) FROM multi_board B" + "		INNER JOIN membership M "
				+ "ON B.id=M.id  WHERE attachedfile is null and notice=?";
	...
	public BoardDTO selectView(String num) {
		...
		String query = "SELECT" + "    num, title, content, " + "DATE_FORMAT(postdate , '%Y-%m-%d') AS pdate, "
				+ "b.id, visitcount, name ,email, attachedfile " + " FROM membership m INNER JOIN multi_board b "
	...
	public int insertWrite(BoardDTO dto) {
		int affected = 0;
		try {
			String sql = "INSERT INTO multi_board (title, content, id, notice,attachedfile) " + 
					"	VALUES (?,?,?,?,?)";
			psmt = con.prepareStatement(sql);
	...
	public List<BoardDTO> selectFileListPageSearch(Map<String, Object> map) {
		List<BoardDTO> bbs = new Vector<BoardDTO>();

		// 쿼리문이 아래와같이 페이지처리 쿼리문으로 변경됨.
		String query = "" + " SELECT IF(char_LENGTH(title)>20, concat(LEFT(title,20),'...'), title) AS title3 ,"
				+ " DATE_FORMAT(postdate , '%Y.%m.%d') AS pdate ," + "b.*, m.name FROM multi_board b "
				+ " INNER JOIN membership m " + " ON b.id=m.id  WHERE attachedfile is not null and notice=?";
		if (map.get("Word") != null) {
			query += " AND " + map.get("Column") + " LIKE '%" + map.get("Word") + "%'";
		}
		query += " ORDER By ";
	...
	public int getTotalFileRecordCountSearch(Map<String, Object> map) {

		// 게시물의 갯수는 최초 0으로 초기화
		int totalCount = 0;
		// 기본쿼리문(전체레코드를 대상으로 함)
		String query = "SELECT COUNT(*) FROM multi_board B" + "		INNER JOIN membership M "
				+ "ON B.id=M.id  WHERE attachedfile is not null and notice=?";
		// JSP페이지에서 검색어를 입력한 경우 where절이 동적으로 추가된다.
		if (map.get("Word") != null) {
			query += " AND " + map.get("Column") + " LIKE '%" + map.get("Word") + "%'";
		}
